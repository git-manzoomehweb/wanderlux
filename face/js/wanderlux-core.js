document.addEventListener('DOMContentLoaded', function () {
  ...
  function postDomFixes() {
    ...
    if (r) r.classList.add('flighttype-dropDown')
  }
  ...
  ;(async function boot() {
    try {
      ...
      console.error('There has been a problem. Please be patient.', e)
      container.innerHTML =
        '<div class="text-center p-3">Error loading. Please try again.</div>'
    }
  })()
})
...
document.addEventListener('DOMContentLoaded', function () {
  if (!document.querySelector('.contain-currency')) {
    if (!document.querySelector('.contain-currency-show')) {
      let headerResize = document.querySelector('#header-resize')
      let currencySelected = headerResize.querySelector('.currency-selected')
      const getCurrencyObject = localStorage.getItem('currencyObject')
      const jsonCurrency = JSON.parse(getCurrencyObject)
      if (!jsonCurrency) {
...
// let tourSwiper = null
// let tourSwiperMobile = null

// function initTourSwipers() {
//   try {
//     tourSwiper?.destroy(true, true)
//   } catch {}
//   try {
//     tourSwiperMobile?.destroy(true, true)
//   } catch {}

//   if (document.querySelector('.tour-swiper-mobile')) {
//     tourSwiperMobile = new Swiper('.tour-swiper-mobile', {
//       slidesPerView: 1.5,
//       speed: 400,
...
//     try {
//       const res = await fetch('/tour-load-items.bc?catid=212998')
//       if (!res.ok) throw new Error(`HTTP error! Status: ${res.status}`)
//       const html = await res.text()
//       fetchContentTour.innerHTML = html
...
//       fetchContentTour.innerHTML = '<div class="loading-text">Loading...</div>'
//       try {
//         const res = await fetch(`/tour-load-items.bc?catid=${cmsQuery}`)
//         if (!res.ok) throw new Error(`HTTP error! Status: ${res.status}`)
//         const html = await res.text()
//         fetchContentTour.innerHTML = html
