...
function addMulticityRoute(t) {
   if (document.querySelector(".route-container").querySelectorAll(".route-content").length < 4) {
      ...
      i.innerHTML = e, $(window).width() >= 1024 ? i.className = "route-content set_Date_Box float-left w-full relative mt-6" : i.className = "route-content float-left relative w-full mt-6", i.querySelector(".multi-route-tlt").innerText = destination_nth_txt[document.querySelector(".route-container").querySelectorAll(".route-content").length], i.querySelectorAll("input").forEach(t => {
         t.value = ""
